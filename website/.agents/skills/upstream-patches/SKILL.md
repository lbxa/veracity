...
description: Use when creating, amending, removing, rebasing, reviewing, or debugging patches for Terraform provider submodules with `./scripts/upstream.sh`. Covers Pulumi provider patch stacks, upstream checkout/check_in flows, owning-patch lookup, patch conflict fixes, hunk removal, upstream rebase, and patch integrity verification during `upgrade-provider` or manual patch work.
---
