...
description: Use when upgrading a Pulumi provider repository to a new upstream Terraform provider version with the `upgrade-provider` tool. Covers provider repo upgrade loops, bridge/provider generation failures, patch conflicts, missing module mappings, go.mod replacement issues, ignored replace directives, unknown revisions, upstream patch review, and post-run verification.
---
