...
version: 1.0.0
description: Use when the user asks how to run Pulumi programmatically, embed Pulumi in an application, orchestrate multiple stacks in code, build a self-service infrastructure portal, replace pulumi CLI shell scripts with code, or use Pulumi Automation API concepts such as LocalWorkspace, createOrSelectStack, stack.up, inline programs, or stack outputs. Also use for multi-stack sequencing, parallel deployments, and passing outputs between stacks via code.
---
