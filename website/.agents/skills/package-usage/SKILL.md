---
name: package-usage
description: Use when auditing which Pulumi Cloud stacks across an organization use a specific package and at what versions. Covers cross-stack package usage, outdated stack inventories, affected-stack discovery before publishing breaking component packages, and coordinated rollout planning. Do NOT use for provider repository upgrades; use `pulumi-upgrade-provider` for those. Do NOT use for ordinary dependency bumps in one project, general infrastructure creation, resource provisioning, or package how-to questions.
---
